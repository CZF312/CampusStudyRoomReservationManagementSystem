...
### 10.4 信用积分算法（由数据库存储过程 `sp_change_student_credit` 实现）

为了保障信用变更与黑名单判定的强一致性事务操作，该核心算法下沉至数据库存储过程实现：

```sql
-- 存储过程核心逻辑伪代码
CREATE PROCEDURE sp_change_student_credit(userId, delta, type, reason, reservationId, threshold, freezeDays)
BEGIN
    START TRANSACTION;
    -- 1. 获取并加排他锁读取信用分
    SELECT credit_score INTO before FROM student_profile WHERE user_id = userId FOR UPDATE;
    
    -- 2. 计算并在区间内限制信用分
    SET after = before + delta;
    IF after < 0 THEN SET after = 0;
    ELSEIF after > 500 THEN SET after = 500;
    END IF;
    
    -- 3. 写入更新与日志记录
    UPDATE student_profile SET credit_score = after WHERE user_id = userId;
    INSERT INTO credit_log(before_score, change_value, after_score, change_type, reason) ...
    
    -- 4. 判定是否扣减至阈值并执行拉黑联动
    IF after <= threshold THEN
        INSERT INTO blacklist_record(user_id, start_time, end_time, reason, status) ...
        UPDATE user_account SET status = '拉黑' WHERE id = userId;
    END IF;
    COMMIT;
END
```

后端 Java 仅通过 `jdbc.update("CALL sp_change_student_credit(?, ?, ?, ?, ?, ?, ?)", ...)` 进行调用。
