...
版本：V3.0（第三版数据字典规范化）  
依据文档：《校园自习室预约管理系统需求分析文档（第三版）》、《原型设计.html》、《2026数据库课程设计任务书》、《数据库课程设计报告模板》、《第三版数据字典规范化实施总结报告》  
适用阶段：应用系统设计阶段  
目标：用于指导后续数据库建表、前后端编码、接口联调、测试和部署演示。本文档已对齐 V3.0 数据库 schema（16 张表 + 1 个视图 + 24 条外键约束）。
...
校园自习室预约管理系统是一个通过浏览器访问的 B/S 架构 Web 应用，面向学生、普通管理员和超级管理员三类用户。系统通过在线预约、学号签到、签退统计、信用积分、公告通知、问题反馈和统计分析，提高自习室座位资源利用率，减少占座、找座和管理困难等问题。
...
| 持久层   | Spring JDBC（JdbcTemplate）                    | 直接编写 SQL，完成数据库 CRUD、分页、复杂查询和报表             |
| 数据库   | MySQL 8                                     | 存储用户、自习室、座位、预约、信用、公告、反馈等数据                 |
| 鉴权    | JWT、Spring Security                         | 学生、普通管理员、超级管理员分角色授权                        |
...
flowchart TB
    ...
    Security["认证授权 JWT + Spring Security"]
    Service["业务服务层 AppService"]
...
| 正常学生  | 可预约、取消未开始预约、查看公告、到馆后由管理员输入学号签到、签退、查看学习统计、提交反馈 |
| 黑名单学生 | 可登录、查看个人信息和历史预约，不能创建新预约                        |
| 普通管理员 | 只能管理本人负责的自习室、座位、预约、输入学号签到、反馈和统计数据；不能新增和删除自习室   |
...
| 登录账号   | 学生为学号，管理员为管理员账号 | 字符串  | 不超过 30 字符                               | 202225220101        |
| 密码     | 登录密码，不明文保存      | 字符串  | BCrypt 后 60-100 字符                      | `$2a$10$...`        |
| 用户角色   | 账号角色            | 字符串  | 学生 / 管理员 / 超级管理员                      | 学生                 |
| 账号状态   | 当前可用状态          | 字符串  | 待审核 / 正常 / 已禁用 / 黑名单                  | 正常                  |
...
| role          | varchar(20)  | not null           | 角色（学生/管理员/超级管理员） |
| status        | varchar(20)  | not null           | 状态（待审核/正常/已禁用/黑名单） |
...
| 学号     | 学生唯一标识          | 字符串  | 12 位数字                        | 202225220101                                              |
| 姓名     | 学生姓名            | 字符串  | 2-10 个汉字或 2-50 字符             | 张三                                                        |
| 性别     | 学生性别            | 字符串  | 男 / 女 / 保密                    | 男                                                         |
| 学院     | 所属学院            | 字符串  | 不超过 50 字符                     | 计算机学院                                                     |
| 专业     | 所属专业            | 字符串  | 不超过 50 字符                     | 软件工程                                                      |
| 年级     | 入学年份            | 字符串  | 4 位数字                         | 2023                                                      |
| 手机号码   | 联系电话            | 字符串  | 11-20 字符                      | 13800138000                                               |
| 邮箱     | 电子邮箱            | 字符串  | 不超过 100 字符                    | [zhangsan@example.edu.cn](mailto:zhangsan@example.edu.cn) |
| 身份材料地址 | 上传的身份证明材料       | 字符串  | 不超过 255 字符                    | /uploads/material/10001.jpg                               |
| 注册审核状态 | 管理员审核结果         | 字符串  | 待审核 / 已通过 / 已拒绝                | 已通过                                                      |
...
| student_no   | varchar(20)  | not null, unique      | 学号（12位） |
| name         | varchar(50)  | not null              | 姓名        |
| gender       | varchar(10)  | not null              | 性别        |
| college      | varchar(50)  | not null              | 学院        |
| major        | varchar(50)  | not null              | 专业        |
| grade        | varchar(10)  | not null              | 年级（4位）   |
...
| 角色    | 管理员角色   | 字符串  | 普通管理员 / 超级管理员             | 普通管理员              |
| 联系电话  | 管理员手机号  | 字符串  | 11-20 字符                   | 13800138000         |
| 账号状态  | 当前状态    | 字符串  | 正常 / 已禁用                   | 正常                  |
| 创建时间  | 账号创建时间  | 日期时间 | YYYY-MM-DD HH:MM:SS        | 2026-04-01 09:00:00 |
...
数据组成：自习室信息 = 自习室编号 + 自习室名称 + 所在位置 + 楼层 + 开放时间 + 当前状态 + 房间类型 + 负责人 + 行数 + 列数 + 布局图片地址 + 创建时间。
...
| 自习室状态  | 当前开放情况      | 枚举   | 开放 / 维护中 / 已关闭 | OPEN                      |
...
| room_code        | varchar(10)  | not null, unique   | 自习室编号   |
...
| room_type        | varchar(10)  | not null, default "普通" | 房间类型 |
...
> **V3.0 新增表**：替代旧版 `study_room.facilities` 逗号拼接字段，支持设施复用。系统初始化时自动插入：空调、WiFi、电源插座、饮水机、白板、投影设备。

| 数据项   | 说明     | 数据类型 | 取值范围/长度 | 示例   |
| ------ | ------ | ---- | --------- | ---- |
| 设施ID  | 设施唯一标识 | 整数   | bigint，自增 | 1    |
| 设施名称  | 设施名称   | 字符串  | 不超过 20 字符 | 空调   |
| 创建时间  | 数据创建时间 | 日期时间 | YYYY-MM-DD HH:MM:SS | 2026-04-01 09:00:00 |
...
| 字段名  | 类型           | 约束                 | 说明     |
| ----- | ------------ | ------------------ | ------ |
| id    | bigint       | PK, auto_increment | 设施ID  |
| name  | varchar(20)  | not null, unique   | 设施名称 |
| created_at | datetime     | not null           | 创建时间 |

索引：`uk_facility_name(name)`。

---


### 4.6 自习室-设施关联表 study_room_facility

数据组成：自习室-设施关联 = 自习室ID + 设施ID（联合主键）。

> **V3.0 新增表**：`facility` 与 `study_room` 的多对多关联表。查询房间详情时 JOIN 此表返回设施列表。

| 数据项   | 说明      | 数据类型 | 取值范围/长度 | 示例   |
| ------ | ------- | ---- | --------- | ---- |
| 自习室ID  | 所属自习室   | 整数   | bigint    | 40001 |
| 设施ID   | 关联设施    | 整数   | bigint    | 1    |

物理设计：

| 字段名     | 类型     | 约束                          | 说明   |
| --------- | ------- | ---------------------------- | ---- |
| room_id   | bigint | PK, FK → study_room(id)      | 自习室ID |
| facility_id | bigint | PK, FK → facility(id)         | 设施ID |

索引：`idx_srf_facility(facility_id)`。
...
| 座位类型    | 座位子类     | 字符串  | 普通 / 静音 / 电源 / 靠窗 / 热门           | 普通                  |
| 是否有电源   | 电源插座属性   | 布尔     | 0/1                        | 1                   |
| 是否靠窗    | 靠窗属性     | 布尔     | 0/1                        | 0                   |
| 是否静音区   | 静音属性     | 布尔     | 0/1                        | 1                   |
| 是否热门    | 热门属性     | 布尔     | 0/1                        | 0                   |
| 座位状态    | 管理员设置状态  | 枚举     | 空闲 / 维修中 / 已禁用 | 正常              |
| 创建时间    | 数据创建时间   | 日期时间   | YYYY-MM-DD HH:MM:SS        | 2026-04-01 09:00:00 |
...
| status        | varchar(20) | not null            | 空闲 / 维修中 / 已禁用 |
| created_at    | datetime    | not null            | 创建时间                       |
| updated_at    | datetime    | not null            | 更新时间                       |
...
| 预约编号    | 页面展示编号      | 字符串  | 16 位纯数字                                                          | 2026042000010001       |
...
| 预约状态    | 当前业务状态      | 枚举   | 待使用 / 使用中 / 已完成 / 已取消 / 已违约 / 自动签退 | 待使用             |
| 签到时间    | 管理员输入学号确认时间   | 日期时间 | YYYY-MM-DD HH:MM:SS                                                | 2026-04-20 08:55:00 |
...
| status         | varchar(20) | not null           | 有效 / 已释放 |
...
| 变动类型   | 业务类型    | 枚举   | 注册初始 / 按时签到 / 未签到 / 自动签退 / 黑名单解除 | NO_CHECKIN          |
...
| 公告类型  | 公告分类    | 枚举   | 系统通知 / 维护公告 / 活动通知 / 使用规则 / 其他 | SYSTEM              |
| 是否置顶  | 首页优先展示  | 布尔   | 0/1                                            | 1                   |
| 发布范围  | 全局或自习室  | 枚举   | 全局 / 指定自习室                                  | GLOBAL              |
| 自习室ID | 指定自习室公告 | 整数   | bigint，可空                                      | 40001               |
| 发布人   | 发布管理员   | 整数   | bigint                                         | 30001               |
| 状态    | 草稿或已发布  | 枚举   | 草稿 / 已发布 / 已删除                    | PUBLISHED           |
...
| 通知类型   | 通知分类      | 枚举   | 预约通知 / 公告通知 / 信用通知 / 系统通知 | RESERVATION         |
| 是否已读   | 阅读状态      | 布尔   | 0/1                                          | 0                   |
...
| 问题类型   | 反馈分类        | 枚举   | 座位维修 / 环境问题 / 系统问题 / 建议 / 其他 | SEAT_REPAIR         |
| 严重程度   | 处理优先级       | 枚举   | 低 / 中 / 高                                     | MEDIUM              |
| 反馈内容   | 学生输入内容      | 字符串  | 4-1000 字符                                               | 座位插座无法使用            |
| 处理状态   | 管理员处理进度     | 枚举   | 待处理 / 处理中 / 已转交 / 已处理               | 待使用             |
...
| 操作人角色  | 操作角色     | 枚举   | 学生 / 管理员 / 超级管理员 | ADMIN               |
...
V3.0 实际只有一个视图 `v_room_daily_usage`，用于统计自习室每日使用率。复杂查询在 Java Service 层通过 JOIN 实现。
...
SELECT 
    r.room_code,
    r.name AS room_name,
    DATE(res.reserve_date) AS usage_date,
    COUNT(DISTINCT res.id) AS total_reservations,
    COUNT(DISTINCT CASE WHEN res.status IN ('已完成', '自动签退') THEN res.id END) AS completed_reservations,
    ROUND(COUNT(DISTINCT CASE WHEN res.status IN ('已完成', '自动签退') THEN res.id END) * 100.0 / NULLIF(COUNT(DISTINCT res.id), 0), 2) AS usage_rate
FROM study_room r
LEFT JOIN seat s ON r.id = s.room_id
LEFT JOIN reservation res ON s.id = res.seat_id
GROUP BY r.id, usage_date;
```

### 5.2 存储过程与触发器说明

V3.0 **未使用**数据库存储过程和触发器。所有业务逻辑在 Java Service 层实现，包括：
- 预约冲突检查（通过数据库唯一索引保证）
- 信用积分计算与限制（0-500分）
- 定时任务（Spring Boot `@Scheduled` 注解）
...
实际包名：`com.scau.campusstudyroomreservationmanagementsystem`

```text
src/main/java/com/scau/campusstudyroomreservationmanagementsystem
├── CampusStudyRoomReservationManagementSystemApplication.java  # 主类
├── config/                                                  # 配置类
│   ├── JwtAuthFilter.java                                 # JWT 认证过滤器
│   ├── JwtService.java                                    # JWT 生成与验证
│   ├── SecurityConfig.java                                # Spring Security 配置
│   ├── UploadStorage.java                                 # 文件上传配置
│   └── WebConfig.java                                   # CORS、静态资源配置
├── controller/                                              # 控制器层
│   ├── AppController.java                                  # 统一接口入口（简化设计）
...
注意：V3.0 采用简化设计，所有接口通过 `AppController` 和 `AppService` 统一处理，未严格按模块拆分 Controller。

### 6.2 数据访问设计

V3.0 使用 **JdbcTemplate**（非 MyBatis），SQL 语句直接写在 Java 代码中或通过 `SqlFragments.java` 管理。

```java
// 示例：使用 JdbcTemplate 查询
@Repository
...
| POST   | `/api/admin/checkin/student-no`      | 学号签到         |
...
| GET    | `/api/admin/statistics/export`      | 导出报表（支持多维度参数筛选与报表类型动态关联导出） |
| GET    | `/api/admin/feedback`               | 反馈列表         |
| PUT    | `/api/admin/feedback/{id}`          | 处理反馈         |
| GET    | `/api/admin/settings/config`        | 获取系统配置（仅限超级管理员，动态加载 system_config.json） |
| POST   | `/api/admin/settings/config`        | 更新系统配置（仅限超级管理员，保存至 system_config.json） |
| GET    | `/api/admin/operation-logs`         | 获取管理员操作审计日志 |
...
7. 后端检查学号是否已存在。
8. 如果学号已存在，返回“该学号已注册”。
9. 如果注册成功，账号状态为 `待使用`，学生资料审核状态为 `待使用`。
...
6. 开启数据库事务。
7. 插入 reservation，状态为 待使用。
8. 批量插入 reservation_slot，状态为 有效。
...
| 点击预约卡片 | 打开预约详情                                  |
| 点击取消预约 | 仅 待使用 且未到开始时间的预约可取消                 |
| 取消成功   | 预约状态变为 已取消，释放时间片                  |
...
3. 校验当前时间早于预约开始时间。
4. 更新预约状态为 `已取消`。
5. 将关联 `reservation_slot` 状态改为 `已释放`。
6. 不扣信用积分。
...
| --- | ---------------------- | ---------- |
| 无预约 | 暂无可签到预约                | 去预约        |
| 待签到 | 预约自习室、座位、时间、签到按钮、反馈按钮 | 出示学号、问题反馈 |
...
1. 学生进入签到页，前端查询当前有效预约：`GET /api/reservations/my?status=待使用,使用中`。
2. 如果存在待签到预约，显示“出示学号签到”按钮。
3. 学生点击按钮，前端显示学号输入框。
4. 学生输入学号，调用 `POST /api/admin/checkin/student-no`。
5. 后端校验学号、预约、时间范围和管理员权限。
6. 签到成功后，预约状态变为 `使用中`，签到时间写入数据库。
...
4. 后端将学生审核状态改为 `已审核`，账号状态改为 `正常`，信用积分初始化为 300。
5. 写入信用积分初始化记录。
6. 点击“拒绝”时必须填写拒绝原因，账号状态保持或改为 `停用`。
7. 学生再次登录时展示审核结果。
...
2. 点击删除前弹出确认。
3. 后端检查该自习室是否存在未完成预约。
4. 若存在 `待使用` 或 `使用中` 预约，拒绝删除并提示“该自习室有未完成预约”。
...
1. 管理员进入签到管理页。
2. 前端输入学号，或在课程设计演示中直接输入学号模拟。
3. 输入学生学号后，调用 `POST /api/admin/checkin/student-no`。
4. 后端校验学号、预约、时间范围和管理员权限。
5. 查找该学生当前时间前后 15 分钟内的 `待使用` 预约。
6. 普通管理员校验预约自习室是否归本人负责。
7. 校验通过后写入签到记录，预约状态变为 `使用中`。
8. 返回“签到成功：学号、姓名、自习室、座位”。
9. 若签到超时、预约不存在、越权或超时，返回失败原因。
...
5. 普通管理员只统计负责自习室。
6. 超级管理员统计所有自习室。
7. 导出报表重构为下拉菜单。管理员在下方选择器筛选所需查看的图表维度（选择报表类型如：座位使用率统计、预约趋势分析、高峰时段分析、学生预约活跃度、学习时长排行、信用积分分布，以及时间范围、特定自习室范围）后，点击下拉菜单中对应的导出选项，将调用 `GET /api/admin/statistics/export?reportType=...&startDate=...&endDate=...&roomId=...`，导出与当前图表查询条件完全对应的动态明细 CSV 数据，实现“所见即所得”的数据分析导出。
...
设置界面采用双标签页（Tab）设计，提供全面系统参数配置与操作审计日志追踪：

1. **系统配置标签页（仅超级管理员可用）**：
   - 提供了对系统核心业务参数的持久化修改能力，数据存储于本地 `system_config.json` 中。
   - **配置字段说明**：
     - **可预约天数** (`reservation_advance_days`)：限制学生最多可提前预约多少天内的座位（如 7 天）。
     - **信用分限制** (`credit_blocked_threshold`)：学生低于该积分值时将被禁止预约（如低于 0 分）。
     - **违约扣除积分** (`credit_violation_penalty`)：被判定违约时扣除的积分（如 50 分）。
     - **取消扣除积分** (`credit_cancel_penalty`)：取消预约时扣除的积分（如 50 分）。
     - **签到奖励积分** (`credit_checkin_reward`)：按时签到获得的积分奖励（如 5 分）。
     - **每日最大预约次数** (`reservation_limit_daily`)：每个学生单日最大可预约座位次数（如 3 次）。
     - **黑名单天数限制** (`blacklist_days`)：学生信用分为0时拉黑的限制时间（如 7 天）。
     - **最大预约时长限制** (`reservation_limit_duration`)：单次预约允许的最长小时数（如 4 小时）。
   - **安全鉴权**：普通管理员访问设置页时显示无权操作且无法获取此接口，杜绝数据越权修改。

2. **操作日志标签页（所有管理员可见，超级管理员可全局审计）**：
   - 实时查询管理员的操作审计日志列表。
   - 记录信息包括操作人姓名、操作模块、具体操作动作（如“账户禁用”、“账户启用”、“撤销违约”、“添加管理员”等）以及操作明细（含具体的时间戳与对象标识）。
   - 数据源于 `operation_log` 表的只读查询。

3. **左下角身份展示菜单交互**：
   - 管理员左下角展示头像、姓名和角色。
   - 点击时会弹出微交互菜单，包含**“切换账号”**与**“安全退出”**两个动作项，为管理员提供灵活的账户切换体验。
...
function createReservation(userId, roomId, seatId, date, start, end):
    ...
    begin transaction
        reservationId = insert reservation(status=待使用)
        for slot in slots:
            insert reservation_slot(reservationId, seatId, slot.start, slot.end, 有效)
            if duplicate key:
                rollback
                throw "该座位当前时段已被预约"
    commit
...
function scanCheckin(adminId, qrToken):
    ...
    reservation = find 待使用 reservation
    ...
    update reservation set status=使用中, sign_in_time=now
...
function checkout(userId, reservationId):
    ...
    validate reservation.status == 使用中
    now = currentTime()
...
```text
1. 处理超时未签到：
   查询 status=待使用 且 当前时间 > reserve_date + start_time + 15分钟 的预约。
   更新为 已违约。
   释放时间片。
   扣 50 分。
   发送违约通知。
...
2. 处理到期未签退：
   查询 status=使用中 且 当前时间 > reserve_date + end_time 的预约。
   自动签退，状态为 自动签退。
...
3. 处理黑名单解除：
   查询 status=有效 且 end_time <= 当前时间 的黑名单。
   更新黑名单为 已释放。
   用户状态改为 正常。
   信用积分恢复为 10。
   写入信用记录。
...
```bash
mysql -u root -p
CREATE DATABASE study_room DEFAULT CHARACTER SET utf8mb4 COL迟到 utf8mb4_unicode_ci;
```
...
或一键：`.\scripts\import-database-local.ps1`（见 [数据库文件说明](../06-部署配置/数据库文件说明.md)）
...
| 角色    | 账号           | 密码       | 说明        |
| ----- | ------------ | -------- | --------- |
| 学生    | 202225220101 | 123456   | 正常学生      |
| 学生    | 202325220201 | 123456   | 待审核学生     |
