...
| `不支持发行版本 21`                                    | JDK 版本过低         | 安装 JDK 20+，设置 `JAVA_HOME` 后重开 PowerShell                                    |
| 在 `frontend` 下运行 `mvnw` 报错                      | 目录错了             | `cd <项目根目录>` 再 `.\mvnw.cmd spring-boot:run` |
