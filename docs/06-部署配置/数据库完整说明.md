...
> **文档版本**：与仓库 `schema.sql`（第三版数据字典规范化，2026-05-29）对齐  
> **适用项目**：CSRRMS V3.0（Spring Boot 3.5 + MySQL 8 + JdbcTemplate）  
...
共 **16 张表**、**1 个视图**、**24 条外键约束**（`schema.sql` 中已声明，导入后由 MySQL 强制引用完整性）。

### 5.1 表清单

| 表名 | 中文含义 | 核心用途 |
|------|----------|----------|
| `user_account` | 登录账号 | 学生/统一认证入口；`username` 为学生学号（12 位） |
| `student_profile` | 学生档案 | 姓名、学院、审核状态、信用分（0–500） |
| `admin_account` | 管理员 | 普通管理员 / 超级管理员 |
| `study_room` | 自习室 | 开放时间、负责人、行列规模、`room_type` 房间类型 |
| `facility` | 设施字典 | 空调、WiFi、电源插座等可复用设施项 |
| `study_room_facility` | 房间-设施关联 | 多对多，替代旧版 `study_room.facilities` 逗号串 |
| `seat` | 座位/网格单元 | 座位号、电源/静音等属性 |
| `reservation` | 预约主表 | 某学生某日某时段占某座；`reservation_no` 为 16 位纯数字 |
...
> **第三版变更**：已移除 `temp_leave`（暂离）表；`study_room` 不再存储 `facilities`、`cell_count`、`seat_count` 物理列；`reservation` 不再存储 `actual_minutes` 物理列（查询时由签到/签退时间差动态计算）。

### 5.2 视图

| 视图名 | 说明 |
|--------|------|
| `v_room_daily_usage` | 按**当天**统计各自习室预约数、使用数、使用率；座位数由 `count(seat)` 动态计算 |
...
erDiagram
  ...
  study_room ||--o{ study_room_facility : "room_id"
  facility ||--o{ study_room_facility : "facility_id"
...
| `role` | varchar(20) | `学生`（数据库存中文） |
| `status` | varchar(20) | `正常` / `禁用` / `待审核` |

### 6.2 `student_profile` — 学生档案

| 字段 | 说明 |
|------|------|
| `student_no` | 学号，12 位数字，唯一 |
| `grade` | 年级，4 位数字（如 `2022`） |
| `audit_status` | `待审核` / `已通过` / `已拒绝` |
| `credit_score` | 信用积分，默认 300，范围 0–500 |
| `material_url` | 注册材料路径 |
...
| 字段 | 说明 |
|------|------|
| `room_code` | 房间编码，≤10 字符，唯一 |
| `room_type` | `普通` / `静音` / `讨论` / `考研专区` |
| `open_time` / `close_time` | 每日可预约时段边界 |
| `status` | `开放` / `关闭` / `维护中` |
| `manager_id` | 负责管理员 `admin_account.id` |
| `row_count` / `col_count` | 座位网格规模；`cell_count` 查询时由 `row_count * col_count` 计算 |
| `layout_image_url` | 平面图 URL |
| 设施 | 通过 `study_room_facility` 关联 `facility` 表；接口同时返回 `facilityNames` 数组与 `facilities` 拼接串 |
...
| 字段 | 说明 |
|------|------|
| `reservation_no` | 业务单号，16 位纯数字，唯一 |
| `reserve_date` | 预约日期 |
...
> `actual_minutes` 不再作为物理列存储；统计时由 `sign_out_time - sign_in_time` 动态计算。
...
| 表 | 要点 |
|----|------|
| `facility` | 设施名称字典（如 `空调`、`WiFi`） |
| `study_room_facility` | `(room_id, facility_id)` 关联 |
| `credit_log` | `change_type` + `change_value` + `before_score` / `after_score` |
| `blacklist_record` | `status`：`生效中` / `已解除` 等中文状态 |
| `announcement` | `scope`：全局 / 指定房间；`pinned` 置顶 |
| `notification_message` | `read_flag` 0/1；`type` 为中文（系统通知/使用规则/维护公告/其他） |
| `feedback_ticket` | `severity` 与 `status` 均为中文枚举 |
| `operation_log` | 管理员操作审计；`operator_name` 冗余保留 |

---

## 7. 状态与业务常量

### 7.1 预约 `reservation.status`（数据库存中文）
...
> 前端 `App.vue` 的 `canonical()` 仍兼容历史英文码值（`PENDING`、`USING` 等）用于展示归一化。

### 7.2 信用相关（`AppService` 常量）

| 常量 | 值 | 说明 |
|------|-----|------|
| 信用上限 | 500 | `CREDIT_SCORE_MAX`（第三版统一 0–500 口径） |
...
| 角色 | 账号 | 密码 | 说明 |
|------|------|------|------|
| 学生 | `202225220101` | `123456` | 陈思源，已审核通过 |
| 学生 | `202325220201` | `123456` | 已审核学生示例 |
| 学生 | `202425220301` | `123456` | 待审核示例（`audit_status=待审核`） |
...
```powershell
# 示例：打包 uploads
Compress-Archive -Path <项目根目录>\uploads -DestinationPath uploads-backup.zip
```
