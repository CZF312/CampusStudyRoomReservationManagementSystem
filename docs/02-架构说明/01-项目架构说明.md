...
> 校园自习室预约管理系统（CSRRMS）V3.0  
> 技术栈：Spring Boot 3.5 + Vue 3 + MySQL 8 + JWT + MyBatis
...
POST /api/reservations { roomId, seatId, date, startTime, endTime }
  ...
  → uk_seat_slot(seat_id, slot_start) 唯一索引保证同时段仅一人成功
```
...
```
POST /api/reservations/{id}/temp-leave  → status: 暂离中
POST /api/reservations/{id}/temp-return → status: 使用中
定时任务：暂离 > 30 分钟 → 自动签退 + 扣信用分
```
...
| `facility` / `study_room_facility`      | 设施字典与房间-设施关联 |
...
> **V3.0 变更**：已移除 `temp_leave` 独立表（暂离状态通过 `reservation.status` 管理）；新增 `facility` 与 `study_room_facility` 表（替代旧版逗号串设施字段）。
...
| 前端开发  | http://localhost:5173（代理 API）    |
| MySQL | localhost:3306 / `study_room_reservation` |


生产部署建议：Nginx 反向代理 + Jar + MySQL，或使用 `docs/06-部署配置/docker-compose.yml`。
...
| 项目 | 选择 | 说明 |
|------|------|------|
| ORM | JdbcTemplate | 主要数据访问层，手写 SQL 灵活可控 |
| 扩展 | MyBatis（pom 中已引入） | 可用于复杂 SQL 映射扩展 |
| 认证 | JWT + Spring Security | 无状态 Token，前端 localStorage 保存 |
| 文件存储 | 本地磁盘 `uploads/` | HTTP 路径由 WebConfig 映射 |
| 测试库 | H2（内存库） | 单元/集成测试用，MODE=MySQL |
