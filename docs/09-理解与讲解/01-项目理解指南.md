...
2. **实现定位表三列** — 除 F1.2 概念卡外，各子项表统一为（表内首行表头）：
   - **所属层**：架构层名称 + 相关概念（名词零基础释义，约 18% 宽）；
   - **链路中位置与代码地址**：代码链接 + 分步链中位置（1、2、3… 每步含工作/输入/输出）+ 总输入 / 总输出 / 失败时（约 28% 宽）；
   - **实现讲解**：③ 底层实现（与上列同序，每步 Lx–Ly 对行号）· ④ 设计取舍 · ⑤ 答辩要点（约 54% 宽）。
...
<colgroup>
  ...
  <tbody>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`BCrypt`=密码单向哈希，库中不存明文；`JWT`=登录成功后的 token；`localStorage`=浏览器存 token；`user_account`=学生账号表；`auditStatus`=待审核/已通过；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式</td>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L40–L40）：小明 POST /auth/login，转发至 loginStudent<br>2（L41–L41）：包装 Service 结果为 ApiResponse JSON<br>3（L42–L42）：return ApiResponse.ok(jwtService...)<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>登录端点独立白名单；与学生/管理员分路径见 F2.2。<br>若不这样做：若在 `login` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：登录失败 HTTP 码？答：多数仍 200，message 中文说明；403 待审核见 Service 抛错。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。<br>问：忘记密码怎么办？答：课设演示账号固定；真实系统应做找回流程，本项未实现。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L184–L189）：L184–207 AppService<br>2（L190–L195）：抛业务异常<br>3（L196–L201）：抛业务异常<br>4（L202–L207）：createToken。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>所有登录安全策略集中 Service，Controller 与 adminLogin 共用 JwtService。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：密码怎么存？答：user_account.password_hash BCrypt，永不回传明文。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：忘记密码怎么办？答：课设演示账号固定；真实系统应做找回流程，本项未实现。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L28–L41）：claims 含 sub=userId、role=STUDENT<br><br>④ 设计取舍：为何放在配置层 Config：鉴权、CORS、JWT 密钥与所有接口相关，放 config 包一处维护。<br>签发集中在一处，Filter 与 Service 共用同一 JwtService 解析/验证逻辑。<br>若不这样做：若每个 Controller 手写 if(token)，漏写一个接口就裸奔；抽成 Filter 后新增接口默认受保护。<br><br>⑤ 答辩要点：问：token 里存密码吗？答：不存，只存 userId 与 role 等非敏感 claims。<br>问：忘记密码怎么办？答：课设演示账号固定；真实系统应做找回流程，本项未实现。</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L29–L47）：读 Header Authorization<br><br>④ 设计取舍：为何放在配置层 Config：鉴权、CORS、JWT 密钥与所有接口相关，放 config 包一处维护。<br>统一鉴权，Controller 用 @AuthenticationPrincipal 取当前用户。<br>若不这样做：若每个 Controller 手写 if(token)，漏写一个接口就裸奔；抽成 Filter 后新增接口默认受保护。<br><br>⑤ 答辩要点：问：哪些接口免鉴权？答：登录注册与 static 资源，见 SecurityConfig 白名单。<br>问：忘记密码怎么办？答：课设演示账号固定；真实系统应做找回流程，本项未实现。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JWT`=登录通行证 token</td>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L52–L52）：bootstrap 刷新后 GET /auth/me 恢复小明或 admin 会话信息<br>2（L53–L53）：包装 Service 结果为 ApiResponse JSON<br>3（L54–L54）：return ApiResponse.ok(service.getCurrentUser(userId))<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>「我是谁」只读接口，支撑刷新恢复与 header 展示，不写库。<br>若不这样做：若在 `me` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：与 login 区别？答：login 验密签发 token；me 只解析已有 token 返回资料。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。<br>问：忘记密码怎么办？答：课设演示账号固定；真实系统应做找回流程，本项未实现。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L209–L212）：L2474–2489 `【F2-2】`<br>2（L213–L216）：抛业务异常<br>3（L217–L220）：抛业务异常<br>4（L221–L224）：role 写 ADMIN/SUPER_ADMIN。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>学生与管理员 JWT role 不同，后端 Security 可限制 /admin/** 仅 ADMIN。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：同一浏览器能同时登学生和管理员吗？答：会覆盖 localStorage token，演示时分开浏览器或先 logout。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2435–L2437）：GitHub L2435 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L2438–L2440）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2441–L2443）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2444–L2446）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2447–L2450）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `onRegisterFile` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `onRegisterFile` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L32–L32）：POST /auth/register/upload，返回 materialUrl<br>2（L33–L33）：FormData 含 registerUpload<br>3（L34–L34）：Service INSERT user_account+profile<br>4（L35–L34）：密码 BCrypt 哈希。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>注册成功不等于可登录，须 F2.3 管理员 approve。<br>若不这样做：若在 `registerUpload` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：PDF 存哪？答：UploadController 存 uploads/ 目录，路径写 student_profile 字段。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2501–L2513）：登录成功后 token 写入 localStorage，后续请求带 Bearer<br>2（L2514–L2526）：FormData 含 registerUpload<br>3（L2527–L2539）：Service INSERT user_account+profile<br>4（L2540–L2554）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>注册成功不等于可登录，须 F2.3 管理员 approve。<br>若不这样做：若在前端 `register` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：PDF 存哪？答：UploadController 存 uploads/ 目录，路径写 student_profile 字段。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`BCrypt`=密码哈希；`JSON`={键:值} 文本格式；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L34–L34）：小李 POST 注册，写入待审核账号与 credit=300<br>2（L35–L35）：包装 Service 结果为 ApiResponse JSON<br>3（L36–L36）：Service INSERT user_account+profile<br>4（L37–L36）：密码 BCrypt 哈希。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>注册成功不等于可登录，须 F2.3 管理员 approve。<br>若不这样做：若在 `register` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：PDF 存哪？答：UploadController 存 uploads/ 目录，路径写 student_profile 字段。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L163–L167）：INSERT user_account + student_profile<br>2（L168–L172）：抛业务异常<br>3（L173–L177）：registerUpload 存 uploads/<br>4（L178–L182）：login 时 auditStatus≠已通过抛 403。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>注册与登录分离，人工审核符合课设流程。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：待审核能登录吗？答：不能，Service 抛 BusinessException 403 中文提示。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">业务层 Service<br>相关概念：`BCrypt`=密码单向哈希，库中不存明文；`JWT`=登录成功后的 token；`localStorage`=浏览器存 token；`user_account`=学生账号表；`auditStatus`=待审核/已通过；`Java`=后端运行时</td>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2938–L2938）：GitHub L2938 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L2939–L2939）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2940–L2940）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2941–L2941）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2942–L2946）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `approve` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `approve` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L746–L748）：UPDATE user_account SET audit_status='已通过'<br>2（L749–L751）：写 operation_log<br>3（L752–L754）：拒绝/禁用走 reject/disable 分支。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>注册与登录解耦，人工审核合规课设流程。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：待审核能登录吗？答：不能，loginStudent 抛 403 中文提示。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2961–L2964）：GitHub L2961 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L2965–L2968）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2969–L2972）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2973–L2976）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2977–L2983）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `submitChangePassword` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `submitChangePassword` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`JWT`=登录通行证 token；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L58–L58）：小明改密码 POST /auth/change-password；public ApiResponse&lt;Void&gt; changePassword(@AuthenticationPrincip…<br>2（L59–L59）：本方法 `@GetMapping`/`@PostMapping` 等注解即 URL 路径<br>3（L60–L60）：方法体通常仅 `return ApiResponse.ok(service.changePassword(...))`，不含 SQL<br>4（L61–L61）：包装 Service 结果为 ApiResponse JSON<br>5（L62–L62）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `changePassword` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`changePassword` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L623–L628）：抛业务异常<br>2（L629–L634）：抛业务异常<br>3（L635–L640）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L641–L646）：抛业务异常<br>5（L647–L653）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`changePassword` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2665–L2665）：小明取消待使用预约，提示扣 50 信用分<br>2（L2666–L2666）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2667–L2667）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2668–L2668）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2669–L2670）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `saveProfile` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `saveProfile` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L65–L65）：GitHub L65 起 `【Fx-y】`+`【行】`：`@RestController` 类<br>2（L66–L66）：包装 Service 结果为 ApiResponse JSON<br>3（L67–L67）：方法体通常仅 `return ApiResponse.ok(service.updateProfile(...))`，不含 SQL<br>4（L68–L68）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L69–L67）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `profile` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`profile` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1521–L1523）：GitHub L1521 起 `【Fx-y】`+`【行】`：`AppService.updateProfile` 内 SQL 字符串 + `jdbcTemplate.query/update`<br>2（L1524–L1526）：常见表 student_profile（扩展资料表）、PUT 更新语义、改密强制登出（防 token 继续可用）<br>3（L1527–L1529）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L1530–L1532）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L1533–L1537）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`updateProfile` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L80–L80）：小明打开预约页，GET /api/rooms 拉自习室列表<br>2（L81–L81）：包装 Service 结果为 ApiResponse JSON<br>3（L82–L82）：方法体通常仅 `return ApiResponse.ok(service.rooms(...))`，不含 SQL<br>4（L83–L83）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L84–L82）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `rooms` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`rooms` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。<br>问：两人同时抢一座？答：reservation_slot 唯一键 uk_seat_slot，后插入者 API 报错「已被预约」。</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L96–L96）：小明选 14:00–16:00，查 A 自习室未被 slot 占用的绿色座位；public ApiResponse&lt;List&lt;Map&lt;String, Object&gt;&gt;&gt; availableS…<br>2（L97–L97）：本方法 `@GetMapping`/`@PostMapping` 等注解即 URL 路径<br>3（L98–L98）：方法体通常仅 `return ApiResponse.ok(service.availableSeats(...))`，不含 SQL<br>4（L99–L99）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L100–L101）：包装 Service 结果为 ApiResponse JSON<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `availableSeats` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`availableSeats` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。<br>问：两人同时抢一座？答：reservation_slot 唯一键 uk_seat_slot，后插入者 API 报错「已被预约」。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L283–L288）：GitHub L283 起 `【Fx-y】`+`【行】`：`AppService.availableSeats` 内 SQL 字符串 + `jdbcTemplate.query/update`<br>2（L289–L294）：常见表 reservation_slot（10 分钟时间片占位）、uk_seat_slot（同座同片唯一）、seat 状态（可用/维修/禁用）<br>3（L295–L300）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L301–L306）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L307–L313）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`availableSeats` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：两人同时抢一座？答：reservation_slot 唯一键 uk_seat_slot，后插入者 API 报错「已被预约」。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`reservation`=预约主表；`reservation_slot`=10 分钟占位表；`uk_seat_slot`=唯一索引防两人同座同时段；`reservation_no`=16 位预约号；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2640–L2641）：表：`reservation`（reservation_no 16 位、status=待使用）、`reservation_slot`（uk_seat_slot 防双占）<br>2（L2642–L2643）：冲突则 BusinessException<br>3（L2644–L2645）：读 `system_config` 最长时长与可预约天数。<br>4（L2646–L2647）：notifyUser 发通知。<br>5（L2648–L2650）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>时间片表实现并发互斥，比仅应用层锁可靠；预约号独立生成规则见 schema 注释。<br>若不这样做：若在前端 `createReservation` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：如何防两人同座同时段？答：插入 reservation_slot 时唯一键冲突即失败。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。<br>问：两人同时抢一座？答：reservation_slot 唯一键 uk_seat_slot，后插入者 API 报错「已被预约」。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`reservation`=预约主表；`reservation_slot`=10 分钟占位表；`uk_seat_slot`=唯一索引防两人同座同时段；`reservation_no`=16 位预约号；`JSON`={键:值} 文本格式；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L105–L105）：小明确认 A-12 后 POST，转发 createReservation；public ApiResponse&lt;Map&lt;String, Object&gt;&gt; createReservation…<br>2（L106–L106）：冲突则 BusinessException<br>3（L107–L107）：包装 Service 结果为 ApiResponse JSON<br>4（L108–L108）：notifyUser 发通知。<br>5（L109–L108）：返回 reservationNo。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>时间片表实现并发互斥，比仅应用层锁可靠；预约号独立生成规则见 schema 注释。<br>若不这样做：若在 `createReservation` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：如何防两人同座同时段？答：插入 reservation_slot 时唯一键冲突即失败。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。<br>问：两人同时抢一座？答：reservation_slot 唯一键 uk_seat_slot，后插入者 API 报错「已被预约」。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L319–L332）：多表写在同一 Spring 事务内，失败则回滚；抛业务异常<br>2（L333–L346）：抛业务异常<br>3（L347–L360）：抛业务异常<br>4（L361–L374）：抛业务异常<br>5（L375–L390）：抛业务异常<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>时间片表实现并发互斥，比仅应用层锁可靠；预约号独立生成规则见 schema 注释。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：如何防两人同座同时段？答：插入 reservation_slot 时唯一键冲突即失败。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：两人同时抢一座？答：reservation_slot 唯一键 uk_seat_slot，后插入者 API 报错「已被预约」。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">业务层 Service<br>相关概念：`reservation`=预约主表；`reservation_slot`=10 分钟占位表；`uk_seat_slot`=唯一索引防两人同座同时段；`reservation_no`=16 位预约号；`Java`=后端运行时</td>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L125–L125）：小明取消「待使用」预约，转发 cancelReservation<br>2（L126–L126）：本方法 `@GetMapping`/`@PostMapping` 等注解即 URL 路径<br>3（L127–L127）：包装 Service 结果为 ApiResponse JSON<br>4（L128–L128）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L129–L128）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `cancel` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`cancel` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L432–L434）：小明取消待使用预约，释放 slot 并扣 credit_cancel_penalty<br>2（L435–L437）：抛业务异常<br>3（L438–L440）：抛业务异常<br>4（L441–L445）：releaseSlots。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>取消惩罚在 Service 可读配置，非写死 SQL。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：30 分钟内扣 10 分？答：代码默认 50，以 system_config 为准（F8 对照表）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2652–L2652）：GitHub L2652 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L2653–L2653）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2654–L2654）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2655–L2655）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2656–L2654）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `loadReservations` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `loadReservations` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`JWT`=登录通行证 token；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L112–L112）：小明「我的预约」GET /reservations/my；public ApiResponse&lt;List&lt;Map&lt;String, Object&gt;&gt;&gt; myReservations(@Auth…<br>2（L113–L113）：本方法 `@GetMapping`/`@PostMapping` 等注解即 URL 路径<br>3（L114–L114）：方法体通常仅 `return ApiResponse.ok(service.myReservations(...))`，不含 SQL<br>4（L115–L115）：包装 Service 结果为 ApiResponse JSON<br>5（L116–L116）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `myReservations` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`myReservations` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1985–L1987）：GitHub L1985 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L1988–L1990）：定义 async `syncCheckinStateFromServer()`<br>3（L1991–L1993）：if (studentPage.value !== 'checkin') return；const tracked = activeReservation.value；if (!tracke…<br>4（L1994–L1996）：const prevId = tracked.id；const prevStatus = tracked.status<br>5（L1997–L2003）：const updated = reservations.value.find(r =&gt; Number(r.id) === Number(pr…；if (!updated) return<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `syncCheckinStateFromServer` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `syncCheckinStateFromServer` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L164–L201）：模板 UI 节点：展示学习统计页对应区域 --&gt;；模板标记：绑定数据或事件到 Vue 实例 --&gt;；@click 触发 `confirmCheckout`；@click 触发 `openFe…<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>预约 UI 与 F3.3 我的预约分离 studentPage，减少单页状态交叉。<br>若不这样做：若在前端 `App.vue` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：灰色座位什么意思？答：维修/禁用或已被 reservation_slot 占用，Service availableSeats 过滤。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3249–L3250）：GitHub L3249 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L3251–L3252）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3253–L3254）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3255–L3256）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3257–L3260）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `refreshCheckinQr` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `refreshCheckinQr` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L10–L10）：GitHub L10 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L11–L11）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L12–L12）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L13–L13）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L14–L18）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `createQrSvg` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `createQrSvg` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JWT`=登录通行证 token</td>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3211–L3217）：GitHub L3211 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L3218–L3224）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3225–L3231）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3232–L3238）：带 JWT 调用后端 REST API<br>5（L3239–L3247）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `triggerPhotoScan` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `triggerPhotoScan` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`JWT`=登录通行证 token；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L332–L332）：管理员扫小明学号，转发 scanCheckin；public ApiResponse&lt;Map&lt;String, Object&gt;&gt; scan(@AuthenticationPrincipal C…<br>2（L333–L333）：本方法 `@GetMapping`/`@PostMapping` 等注解即 URL 路径<br>3（L334–L334）：包装 Service 结果为 ApiResponse JSON<br>4（L335–L335）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L336–L335）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `scan` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`scan` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L989–L1001）：管理员输入小明学号，窗口±15min 内改「使用中」+5 分。<br>2（L1002–L1014）：抛业务异常<br>3（L1015–L1027）：抛业务异常<br>4（L1028–L1040）：抛业务异常<br>5（L1041–L1057）：抛业务异常<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>全部业务规则与 SQL 在 Service，管理员 UI 只传 id 与学号。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：能替别人签到吗？答：Service 校验 studentNo 与预约 user_id 一致。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">业务层 Service<br>相关概念：`Java`=后端运行时；checkin_record.sign_in_time；status 使用中；credit +5</td>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L140–L140）：小明 POST 签退，转发 checkout<br>2（L141–L141）：包装 Service 结果为 ApiResponse JSON<br>3（L142–L142）：return ApiResponse.ok(service.checkout(id, userId))<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>REST 资源动词 checkout 挂在预约 id 下；鉴权在 Filter，Controller 不重复验归属。<br>若不这样做：若在 `checkout` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：为何 path 带 id？答：REST 风格，一条预约一次签退，Service 内再验 userId 归属。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L486–L491）：小明签退，status→已完成，记录 actualMinutes；抛业务异常<br>2（L492–L497）：抛业务异常<br>3（L498–L504）：releaseSlots 释放 reservation_slot<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>签退才计入学习统计；仅待使用不算时长，答辩常考点。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：不签退会怎样？答：F4.3 定时 autoCheckout 过 end_time 自动已完成。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L146–L146）：小明签退后查 credit_log 流水与当前积分<br>2（L147–L147）：包装 Service 结果为 ApiResponse JSON<br>3（L148–L148）：DTO 含 score 与 logs 列表。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>读操作无 side effect，适合 GET；分数变更只在签退/取消/违约等 POST 业务里发生。<br>若不这样做：若在 `credit` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：信用存在哪张表？答：user_account.credit_score 权威值，credit_log 存每次变动流水。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L22–L31）：cron 每分钟依次执行无效签到/违约/自动签退/黑名单解除<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>用 Java 定时替代 MySQL EVENT，逻辑可调试、与业务 Service 复用 SQL。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：服务停掉定时还跑吗？答：不跑，必须 Boot 进程在线；答辩说明演示机需保持 8080 运行。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：关掉 8080 定时还跑吗？答：不跑，@Scheduled 随 JVM 进程，答辩演示需保持服务在线。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">业务层 Service<br>相关概念：`Java`=后端运行时；异常 checkin 数据纠正</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1630–L1642）：GitHub L1630 起 `【Fx-y】`+`【行】`：`AppService.scheduledProcessInvalidCheckin` 或 ScheduledTaskService<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>Java @Scheduled 替代 MySQL EVENT/触发器，逻辑可调试、答辩可指 AppService 行号。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：关掉 8080 还跑吗？答：不跑，定时随 JVM；演示需保持 CSRRMS-Backend 窗口在线。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：关掉 8080 定时还跑吗？答：不跑，@Scheduled 随 JVM 进程，答辩演示需保持服务在线。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1644–L1650）：GitHub L1644 起 `【Fx-y】`+`【行】`：`AppService.scheduledProcessNoShow` 或 ScheduledTaskService<br>2（L1651–L1657）：JdbcTemplate UPDATE/INSERT<br>3（L1658–L1666）：status 须为 schema 中文枚举。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>Java @Scheduled 替代 MySQL EVENT/触发器，逻辑可调试、答辩可指 AppService 行号。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：关掉 8080 还跑吗？答：不跑，定时随 JVM；演示需保持 CSRRMS-Backend 窗口在线。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：关掉 8080 定时还跑吗？答：不跑，@Scheduled 随 JVM 进程，答辩演示需保持服务在线。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1668–L1672）：GitHub L1668 起 `【Fx-y】`+`【行】`：`AppService.scheduledProcessAutoCheckout` 或 ScheduledTaskService<br>2（L1673–L1677）：JdbcTemplate UPDATE/INSERT<br>3（L1678–L1682）：status 须为 schema 中文枚举。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>Java @Scheduled 替代 MySQL EVENT/触发器，逻辑可调试、答辩可指 AppService 行号。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：关掉 8080 还跑吗？答：不跑，定时随 JVM；演示需保持 CSRRMS-Backend 窗口在线。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：关掉 8080 定时还跑吗？答：不跑，@Scheduled 随 JVM 进程，答辩演示需保持服务在线。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1684–L1690）：GitHub L1684 起 `【Fx-y】`+`【行】`：`AppService.scheduledProcessBlacklistRelease` 或 ScheduledTaskServ…<br>2（L1691–L1698）：JdbcTemplate UPDATE/INSERT<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>Java @Scheduled 替代 MySQL EVENT/触发器，逻辑可调试、答辩可指 AppService 行号。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：关掉 8080 还跑吗？答：不跑，定时随 JVM；演示需保持 CSRRMS-Backend 窗口在线。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：关掉 8080 定时还跑吗？答：不跑，@Scheduled 随 JVM 进程，答辩演示需保持服务在线。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L512–L521）：public Map&lt;String, Object&gt; myStudyDuration(CurrentUser user, String per…<br>2（L522–L531）：时长=TIMESTAMPDIFF(sign_in, sign_out 或 slot_end)。年报 GROUP BY 须写 `date_format(...,'%%Y-%%m')` 防 Ja…<br>3（L532–L541）：GET `/statistics/my-study-duration`。<br>4（L542–L551）：Service 按 `statsDateWhereCondition` 拼 WHERE。<br>5（L552–L561）：JDBC 聚合 `series`。<br>6（L562–L571）：`studyBars` 转小时。<br>7（L572–L586）：`bar-chart-lite` 渲染。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>学生端两个 `type=date` + `stats-date-popper-single`，避免 daterange 双面板重叠；与管理端共用日期算法。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：有预约但年报全 0？答：查是否仅「待使用」未签到；或接口 500 看 SQL 转义；问：当期和往期区别？答：current 含今日默认窗，past 查历史自选区间。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。<br>问：为什么有预约但统计为 0？答：统计只计「已签到且使用中/已完成」的时长，仅「待使用」不算。</td>
    </tr>
    <tr>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L508–L509）：模板标记：绑定数据或事件到 Vue 实例 --&gt;；模板 UI 节点：展示学习统计页对应区域 --&gt;<br>2（L510–L511）：模板标记：绑定数据或事件到 Vue 实例 --&gt;<br>3（L512–L513）：模板标记：绑定数据或事件到 Vue 实例 --&gt;<br>4（L514–L515）：模板 UI 节点：展示学习统计页对应区域 --&gt;<br>5（L516–L517）：模板标记：绑定数据或事件到 Vue 实例 --&gt;<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `App.vue` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `App.vue` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1958–L1962）：INSERT INTO notification_message(user_id,title,content,is_read,created_at) VALUES(...,0,NOW())<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>同步写入，课设规模够用；未做推送以降低复杂度。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：实时吗？答：非实时；我的预约页 2 分钟 refresh 或手动刷新可见新通知。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2735–L2735）：小明点「全部已读」POST /notifications/read-all<br>2（L2736–L2736）：带 JWT 调用后端 REST API<br>3（L2737–L2737）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2738–L2738）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2739–L2742）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `submitFeedback` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `submitFeedback` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L608–L609）：GitHub L608 起 `【Fx-y】`+`【行】`：`AppService.createFeedback` 内 SQL 字符串 + `jdbcTemplate.query/update`<br>2（L610–L611）：常见表 feedback_ticket（工单表）、status 待处理/已处理<br>3（L612–L613）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L614–L615）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L616–L617）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`createFeedback` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3282–L3283）：GitHub L3282 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L3284–L3285）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3286–L3287）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3288–L3289）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3290–L3293）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `submitFeedbackHandle` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `submitFeedbackHandle` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1501–L1503）：GitHub L1501 起 `【Fx-y】`+`【行】`：`AppService.handleFeedback` 内 SQL 字符串 + `jdbcTemplate.query/updat…<br>2（L1504–L1506）：抛业务异常<br>3（L1507–L1509）：抛业务异常<br>4（L1510–L1512）：抛业务异常<br>5（L1513–L1519）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`handleFeedback` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L518–L581）：模板标记：绑定数据或事件到 Vue 实例 --&gt;；模板 UI 节点：展示学习统计页对应区域 --&gt;；@click 触发 `changeAdminStatsRangeMode('past')`…<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>弃用 daterange，两个 type=date + stats-date-popper-single，从 UI 层消除图层重叠。<br>若不这样做：若在前端 `App.vue` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何柱图不用 ECharts 为主？答：lite 柱图 CSS 足够答辩；drawStudentChart 保留扩展。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L554–L556）：模板标记：绑定数据或事件到 Vue 实例 --&gt;<br>2（L557–L559）：模板标记：绑定数据或事件到 Vue 实例 --&gt;<br>3（L560–L562）：模板 UI 节点：展示学习统计页对应区域 --&gt;；@click 触发 `changeAdminStatsRangeMode('past')`<br>4（L563–L565）：@click 触发 `changeAdminStatsPeriod('day')`<br>5（L566–L572）：@click 触发 `changeAdminStatsPeriod('week')`；@click 触发 `changeAdminStatsPeriod('month')`；@click 触…<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `el-date-picker` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `el-date-picker` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3342–L3342）：GitHub L3342 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L3343–L3343）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3344–L3344）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3345–L3345）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3346–L3350）：初始化 GET 查询参数字典，键名与后端约定一致；带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `loadAdminStatistics` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `loadAdminStatistics` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JDBC 动态 SQL`=Java 字符串拼 SELECT，非查视图；`statisticsUsage`=各室使用率；`exportCsv`=与报表同 SQL 导出；`JSON`={键:值} 文本格式；`ApiResponse`={code,data,message} 统一响应；`ECharts`=图表库；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L388–L389）：public ApiResponse&lt;Map&lt;String, Object&gt;&gt; statisticsReport(@Authenticatio…<br>2（L390–L391）：动态 SQL 按 period、rangeMode、roomId（普管仅本室）拼接<br>3（L392–L395）：包装 Service 结果为 ApiResponse JSON<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>report 与 export 共用算法，避免「图一套数、CSV 另一套数」；视图仅验收展示今日使用率。<br>若不这样做：若在 `statisticsReport` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：报表从视图来吗？答：不是，AppService JDBC 现算；视图只有 v_room_daily_usage 且管理端报表链未引用。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1239–L1252）：public Map&lt;String, Object&gt; statisticsReport(CurrentUser admin, String p…<br>2（L1253–L1266）：动态 SQL 按 period、rangeMode、roomId（普管仅本室）拼接<br>3（L1267–L1281）：6 种 reportType：usage/peak/reservation/activity/studyDuration/credit。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>report 与 export 共用算法，避免「图一套数、CSV 另一套数」；视图仅验收展示今日使用率。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：报表从视图来吗？答：不是，AppService JDBC 现算；视图只有 v_room_daily_usage 且管理端报表链未引用。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1103–L1105）：public List&lt;Map&lt;String, Object&gt;&gt; statisticsUsage(CurrentUser admin, Str…<br>2（L1106–L1108）：常见表 JDBC 动态 SQL（非 SELECT * FROM 视图）、statisticsReport、exportCsv<br>3（L1109–L1111）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L1112–L1114）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L1115–L1121）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`statisticsUsage` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">业务层 Service<br>相关概念：`JdbcTemplate`=Spring 执行 SQL；`Java`=后端运行时；rangeMode；statPeriod；studyStats ref；studyBars</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1135–L1136）：public List&lt;Map&lt;String, Object&gt;&gt; statisticsPeak(CurrentUser admin, Stri…<br>2（L1137–L1138）：常见表 JDBC 动态 SQL（非 SELECT * FROM 视图）、statisticsReport、exportCsv<br>3（L1139–L1140）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L1141–L1142）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L1143–L1146）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`statisticsPeak` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3352–L3355）：管理员统计页 loadAdminStatistics 拉取报表<br>2（L3356–L3359）：初始化 GET 查询参数字典，键名与后端约定一致<br>3（L3360–L3363）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3364–L3367）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3368–L3371）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `downloadReport` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `downloadReport` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`JWT`=登录通行证 token；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L402–L402）：GET `/admin/statistics/export` 路由<br>2（L403–L403）：public ResponseEntity&lt;byte[]&gt; export(@AuthenticationPrincipal CurrentUs…<br>3（L404–L404）：方法体通常仅 `return ApiResponse.ok(service.export(...))`，不含 SQL<br>4（L405–L405）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L406–L410）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `export` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`export` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1717–L1721）：public String exportCsv(CurrentUser admin, String period, Long roomId, …<br>2（L1722–L1726）：HttpServletResponse 写 BOM+header<br>3（L1727–L1732）：6 种 reportType 与图表一致。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>导出与图表共用算法，答辩「数和图对不上」不成立。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：CSV 从视图来吗？答：否，与 F6.1 一样现算 JDBC。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L568–L579）：@click 触发 `changeAdminStatsPeriod('year')`；v-model 绑定 `adminStatsDateRange`<br>2（L580–L591）：@click 触发 `resetAdminStatsDateRange`；@click 触发 `switchStatAdminView('usage')`；@click 触发 `switch…<br>3（L592–L603）：@click 触发 `switchStatAdminView('share')`<br>4（L604–L615）：v-model 绑定 `feedbackKeyword`；@click 触发 `feedbackStatusFilter = f.key`；@click 触发 `openFeedbackHa…<br>5（L616–L627）：模板 UI 节点：展示学习统计页对应区域 --&gt;；模板标记：绑定数据或事件到 Vue 实例 --&gt;<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `App.vue` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `App.vue` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2804–L2806）：GitHub L2804 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L2807–L2809）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2810–L2812）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2813–L2815）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2816–L2821）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `saveSystemConfig` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `saveSystemConfig` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L125–L125）：GitHub L125 起 `【Fx-y】`+`【行】`：`AppService.getSystemConfig` 内 SQL 字符串 + `jdbcTemplate.query/updat…<br>2（L126–L126）：常见表 system_config.json（超管可改 JSON 配置）、最长预约时长 等键值<br>3（L127–L127）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L128–L128）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L129–L129）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`getSystemConfig` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">业务层 Service<br>相关概念：`JdbcTemplate`=Spring 执行 SQL；`JSON`={键:值} 文本格式；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L131–L134）：superadmin 打开配置页，读取 system_config.json 当前规则；if (newConfig == null) return<br>2（L135–L138）：常见表 system_config.json（超管可改 JSON 配置）、最长预约时长 等键值<br>3（L139–L142）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L143–L146）：抛业务异常<br>5（L147–L151）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`updateSystemConfig` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L364–L382）：@click 触发 `openFeedbackModal`；@click 触发 `aboutOpen = true`；@click 触发 `studentPage = n.page`<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>预约 UI 与 F3.3 我的预约分离 studentPage，减少单页状态交叉。<br>若不这样做：若在前端 `App.vue` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：灰色座位什么意思？答：维修/禁用或已被 reservation_slot 占用，Service availableSeats 过滤。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2985–L2985）：GitHub L2985 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L2986–L2986）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2987–L2987）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2988–L2988）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2989–L2993）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `confirmReject` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `confirmReject` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`JWT`=登录通行证 token；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L240–L240）：管理员拒绝注册 POST reject，auditUser(approve=false)；public ApiResponse&lt;Void&gt; reject(@AuthenticationPri…<br>2（L241–L241）：本方法 `@GetMapping`/`@PostMapping` 等注解即 URL 路径<br>3（L242–L242）：方法体通常仅 `return ApiResponse.ok(service.reject(...))`，不含 SQL<br>4（L243–L243）：包装 Service 结果为 ApiResponse JSON<br>5（L244–L244）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `reject` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`reject` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2995–L2995）：管理员拒绝小李注册，POST /admin/users/{id}/reject<br>2（L2996–L2996）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2997–L2997）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2998–L2998）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2999–L3003）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `disable` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `disable` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`JWT`=登录通行证 token；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L248–L248）：禁用学生账号 POST disable<br>2（L249–L249）：本方法 `@GetMapping`/`@PostMapping` 等注解即 URL 路径<br>3（L250–L250）：包装 Service 结果为 ApiResponse JSON<br>4（L251–L251）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L252–L251）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `disable` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`disable` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2902–L2902）：GitHub L2902 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L2903–L2903）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2904–L2904）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2905–L2905）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2906–L2907）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `loadUsers` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `loadUsers` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2909–L2911）：管理员打开用户管理 GET /admin/users 带 keyword/auditStatus 筛选；带 JWT 调用后端 REST API<br>2（L2912–L2914）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2915–L2917）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2918–L2920）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2921–L2925）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `exportUsersCsv` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `exportUsersCsv` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3005–L3005）：管理员禁用违规学生账号 POST /admin/users/{id}/disable<br>2（L3006–L3006）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3007–L3007）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3008–L3008）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3009–L3013）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `enable` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `enable` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L412–L434）：管理员审核/拒绝/禁用学生，可导出 CSV --&gt;；模板标记：绑定数据或事件到 Vue 实例 --&gt;；模板 UI 节点：展示学习统计页对应区域 --&gt;<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>预约 UI 与 F3.3 我的预约分离 studentPage，减少单页状态交叉。<br>若不这样做：若在前端 `App.vue` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：灰色座位什么意思？答：维修/禁用或已被 reservation_slot 占用，Service availableSeats 过滤。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3040–L3045）：GitHub L3040 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L3046–L3051）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3052–L3057）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3058–L3063）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3064–L3071）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `saveRoom` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `saveRoom` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L762–L772）：保存自习室并同步 row×col 座位网格；抛业务异常<br>2（L773–L783）：抛业务异常<br>3（L784–L794）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L795–L805）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L806–L817）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`saveRoom` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3081–L3081）：superadmin DELETE /admin/rooms/{id} 删除空自习室<br>2（L3082–L3082）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3083–L3083）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3084–L3084）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3085–L3087）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `loadAdminSeats` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `loadAdminSeats` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3073–L3073）：GitHub L3073 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L3074–L3074）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3075–L3075）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3076–L3076）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3077–L3079）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `deleteRoom` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `deleteRoom` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3326–L3328）：GitHub L3326 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L3329–L3331）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3332–L3334）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3335–L3337）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3338–L3340）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `deleteSeatEdit` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `deleteSeatEdit` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L441–L452）：模板标记：绑定数据或事件到 Vue 实例 --&gt;<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>预约 UI 与 F3.3 我的预约分离 studentPage，减少单页状态交叉。<br>若不这样做：若在前端 `App.vue` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：灰色座位什么意思？答：维修/禁用或已被 reservation_slot 占用，Service availableSeats 过滤。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token；`ECharts`=图表库</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2757–L2761）：根据最新 studyBars 重绘 ECharts 柱图<br>2（L2762–L2766）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2767–L2771）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L2772–L2776）：带 JWT 调用后端 REST API<br>5（L2777–L2784）：模板自动刷新 DOM；全程不直连 MySQL。<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `openAdmin` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `openAdmin` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L3305–L3306）：GitHub L3305 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L3307–L3308）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L3309–L3310）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L3311–L3312）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L3313–L3315）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `submitRevokeViolation` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `submitRevokeViolation` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L323–L324）：撤销违约 POST /admin/reservations/{id}/revoke-violation；public ApiResponse&lt;Map&lt;String, Object&gt;&gt; rev…<br>2（L325–L326）：写 operation_log。<br>3（L327–L328）：包装 Service 结果为 ApiResponse JSON<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>人工纠错通道，避免误违约永久黑。<br>若不这样做：若在 `revokeViolation` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：谁可操作？答：管理端 F6.5，需 ADMIN JWT。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L931–L947）：管理员撤销违约，恢复信用并写 operation_log；抛业务异常<br>2（L948–L964）：抛业务异常<br>3（L965–L983）：status 回待使用或已取消视规则。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>人工纠错通道，避免误违约永久黑。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：谁可操作？答：管理端 F6.5，需 ADMIN JWT。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L206–L206）：签到页 GET /admin/live-reservations<br>2（L207–L207）：包装 Service 结果为 ApiResponse JSON<br>3（L208–L208）：方法体通常仅 `return ApiResponse.ok(service.liveReservations(...))`，不含 SQL<br>4（L209–L209）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L210–L208）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `liveReservations` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`liveReservations` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L659–L660）：GitHub L659 起 `【Fx-y】`+`【行】`：`AppService.liveReservations` 内 SQL 字符串 + `jdbcTemplate.query/upda…<br>2（L661–L662）：常见表 live-reservations（实时列表）、使用中/待使用 混合看板<br>3（L663–L664）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L665–L666）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L667–L672）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`liveReservations` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`MySQL`=关系型数据库；`Vue`=前端单页框架；`JSON`={键:值} 文本格式</td>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L388–L406）：@click 触发 `switchAdminAccount`；@click 触发 `logout`；@click 触发 `openAdmin(n.page)`<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>预约 UI 与 F3.3 我的预约分离 studentPage，减少单页状态交叉。<br>若不这样做：若在前端 `App.vue` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：灰色座位什么意思？答：维修/禁用或已被 reservation_slot 占用，Service availableSeats 过滤。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">表现层 Presentation<br>相关概念：`localStorage`=浏览器存 token；`MySQL`=关系型数据库；`Vue`=前端单页框架；`REST`=URL+HTTP 动词；`JSON`={键:值} 文本格式；`JWT`=登录通行证 token</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L2844–L2847）：GitHub L2844 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L2848–L2851）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L2852–L2855）：带 JWT 调用后端 REST API<br>4（L2856–L2859）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L2860–L2866）：带 JWT 调用后端 REST API<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `saveAdminAccount` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `saveAdminAccount` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1566–L1569）：GitHub L1566 起 `【Fx-y】`+`【行】`：`AppService.createAdminAccount` 内 SQL 字符串 + `jdbcTemplate.query/u…<br>2（L1570–L1573）：抛业务异常<br>3（L1574–L1577）：抛业务异常<br>4（L1578–L1581）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L1582–L1585）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`createAdminAccount` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">接口层 Controller<br>相关概念：`JSON`={键:值} 文本格式；`JWT`=登录通行证 token；`ApiResponse`={code,data,message} 统一响应；`Java`=后端运行时</td>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L445–L445）：superadmin GET /admin/operation-logs<br>2（L446–L446）：包装 Service 结果为 ApiResponse JSON<br>3（L447–L447）：方法体通常仅 `return ApiResponse.ok(service.operationLogs(...))`，不含 SQL<br>4（L448–L448）：当前用户从 `@AuthenticationPrincipal` 或 SecurityContext 取 userId。<br>5（L449–L447）：把 Service 返回值包进 ApiResponse 写回浏览器。<br><br>④ 设计取舍：为何放在接口层 Controller：HTTP 形态（路径、动词、状态码）集中在一层，Service 可被 Controller、定时任务、脚本测试共用。<br>Controller 像「前台接待」：只懂 HTTP，不懂预约规则；所有 if/else 与 SQL 在 Service，定时任务也能调同一 Service。<br>若不这样做：若在 `operationLogs` 写 JDBC，则定时任务、导出、多处入口要复制 SQL，一改规则漏改一处就出 bug，故只做 HTTP 适配。<br><br>⑤ 答辩要点：问：`operationLogs` 有多厚？答：答辩可指行号证明几乎只有一行 return service；问：会做鉴权吗？答：JwtAuthFilter 在进 Controller 前已完成，本方法只取已解析的 userId。<br>问：这行会访问数据库吗？答：Controller 层一般不写 SQL，查表在下一行 Service 同名方法。</td>
    </tr>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1539–L1539）：GitHub L1539 起 `【Fx-y】`+`【行】`：`AppService.operationLogs` 内 SQL 字符串 + `jdbcTemplate.query/update`<br>2（L1540–L1540）：常见表 operation_log、admin_account CRUD、superadmin 专属<br>3（L1541–L1541）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L1542–L1542）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L1543–L1545）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`operationLogs` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
  </tbody>
</table>
...
<colgroup>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L384–L384）：按 10 分钟切 slot；唯一索引防 A-12 被双人占<br>2（L385–L385）：抛业务异常<br>3（L386–L386）：status 字段值必须与 schema.sql 第三版中文 CHECK 一致<br>4（L387–L387）：失败 `throw new BusinessException("中文原因")`，不会返回 half-done 数据。<br>5（L388–L386）：返回 Map/DTO 给 Controller。<br><br>④ 设计取舍：为何放在业务层 Service：业务规则 + SQL 是系统「真相来源」，放一层便于单元测试与答辩时打开 AppService 指 SQL。<br>业务规则只写一份在 Service：Controller、@Scheduled、导出 CSV 都调这里，改规则不会漏改某一层。<br>若不这样做：若用 JPA 全自动 SQL，动态报表（F6.1）与复杂 WHERE（F5.1 时间窗）难写难讲，故课选用 JdbcTemplate 显式 SQL 便于答辩指行。<br><br>⑤ 答辩要点：问：为何不用 JPA？答：F6.1/F5.1 动态 SQL 多，JDBC 字符串答辩时可逐行指；问：`DuplicateKeyException` 会事务回滚吗？答：关键写操作在 Spring @Transactional 边界内（见方法注解）。<br>问：SQL 在哪看？答：AppService.java 搜方法名，或 GitHub 定位列 L 行 【行】 注释旁。</td>
    </tr>
    <tr>
      <td style="vertical-align:top;word-break:break-word;">业务层 Service<br>相关概念：`@Scheduled`=Java 定时任务；`Java`=后端运行时；@Scheduled 总入口；串联 markNoShow/autoCheckout 等</td>
      <td style="vertical-align:top;word-break:break-word;">`runMaintenanceTasks` · 定时入口 · `service/ScheduledTaskService.java` · [L22-L31](https://github.com/CZF312/CampusStudyRoomReservationManagementSystem/blob/master/src/main/java/com/scau/campusstudyroomreservationmanagementsystem/service/ScheduledTaskService.java#L22-L31)<br>链中位置：<br>1. Spring @Scheduled 定时（如每分钟）：依次 markNoShow、autoCheckout、解除过期黑名单等 F4.3 维护。<br> 输入：Controller 传入 DTO/基本类型<br> 输出：业务校验通过，可读写 MySQL<br>总输入：@Scheduled 定时触发，无 HTTP 参数。<br>总输出：依次调用 markNoShow/autoCheckout 等维护方法。<br>失败时：单任务异常记日志，下次调度继续。</td>
...
<colgroup>
  ...
  <tbody>
    <tr>
      ...
      <td style="vertical-align:top;word-break:break-word;line-height:1.45;">③ 底层实现：1（L1205–L1205）：GitHub L1205 起 `【Fx-y】`+`【行】`：`App.vue` 中 async function 或模板 `@click`/`@change`<br>2（L1206–L1206）：`call()` 从 localStorage 取 token 加 Authorization Bearer<br>3（L1207–L1207）：成功取 res.data，失败 notify/ElMessage 显示后端 message<br>4（L1208–L1208）：块首 `【Fx-y】` 讲整体，块内 `【行】` 逐行。<br>5（L1209–L1210）：const width = ref(window.innerWidth)；const isDesktop = computed(() =&gt; width.value &gt;= 900)<br><br>④ 设计取舍：为何放在表现层 Presentation：交互密集、改 UI 频繁，放 Vue 模板/JS 最快；与后端通过 REST 解耦，前端可独立 npm run build。<br>前端是「遥控器」：只发请求和展示；信用、时长、status 权威值以后端为准，避免换浏览器结果不一致。<br>若不这样做：若在前端 `RES_STATUS_MAP` 里算信用/写 SQL，换浏览器结果可能不一致，且规则易泄露，故必须只展示后端返回值。<br><br>⑤ 答辩要点：问：为何 `RES_STATUS_MAP` 不算信用/时长？答：安全与一致性；分数在 user_account，由 Service 改，前端只 GET 展示。<br>问：数据存在浏览器吗？答：仅 token 与 UI 状态在 localStorage/ref；预约/信用等权威数据每次从 API 拉。</td>
    </tr>
    <tr>
