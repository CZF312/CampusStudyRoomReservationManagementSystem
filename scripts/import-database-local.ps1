# Import CSRRM database from repo SQL (drop DB then restore by default)
# Usage: .\scripts\import-database-local.ps1
#        .\scripts\import-database-local.ps1 -Password 123456
#        .\scripts\import-database-local.ps1 -UseFullDump -CleanDatabase:$false

param(    [string]$DbHost = "127.0.0.1",
    [int]$Port = 3306,
    [string]$User = "root",
    [string]$Password = "",
    [switch]$UseFullDump,
...
if (-not $cfgDir) {
    $cfgDir = Join-Path $root "docs\06-deploy"
}
...
$initLeaderSql = Join-Path $cfgDir "init-local-database.sql"
