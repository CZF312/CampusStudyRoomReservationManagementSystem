...
function Get-DeployConfigDir([string]$projectRoot) {
    ...
    return Join-Path $projectRoot "docs\06-deploy"
}

Write-Host ""
Write-Host "========================================" -ForegroundColor Green
Write-Host " CSRRM Environment Setup" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
...
if (-not (Test-Path $staticIndex)) {
    ...
    if (Get-Command npm -ErrorAction SilentlyContinue) {
        & (Join-Path $scriptRoot "build-frontend.ps1")
    } else {
...
# Backslashes in properties files can trigger escape sequences; use forward slashes.
...
if (-not $SkipImport) {
    ...
    & (Join-Path $scriptRoot "import-database-local.ps1") -Password $MySqlPassword -User $User -DbHost $DbHost -Port $Port -UseFullDump
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & (Join-Path $scriptRoot "verify-v3-dictionary.ps1") -Password $MySqlPassword -User $User -DbHost $DbHost -Port $Port
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
} else {
