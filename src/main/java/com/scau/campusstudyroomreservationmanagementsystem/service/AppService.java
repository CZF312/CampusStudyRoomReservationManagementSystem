...
public class AppService {
    ...
    public void updateSystemConfig(CurrentUser admin, Map<String, String> newConfig) {
        ...
        try {
            Integer.parseInt(newConfig.getOrDefault("reservation_advance_days", "7"));
            Integer.parseInt(newConfig.getOrDefault("reservation_limit_duration", "4"));
            Integer.parseInt(newConfig.getOrDefault("reservation_limit_daily", "3"));
            Integer.parseInt(newConfig.getOrDefault("credit_checkin_reward", "5"));
            Integer.parseInt(newConfig.getOrDefault("credit_cancel_penalty", "-50"));
            Integer.parseInt(newConfig.getOrDefault("credit_violation_penalty", "-50"));
            Integer.parseInt(newConfig.getOrDefault("credit_blocked_threshold", "0"));
            Integer.parseInt(newConfig.getOrDefault("blacklist_days", "7"));
        } catch (NumberFormatException e) {
            throw new BusinessException(400, "参数格式错误，必须为整数");
        }

        configMap.putAll(newConfig);
        saveConfigToFile();
        writeOperationLog(admin, "ADMIN", "UPDATE", "USER", null, "修改全局预约和积分规则参数");
    }
    ...
    public Map<String, Object> createReservation(CurrentUser user, Map<String, Object> req) {
        ...
        if (date.isBefore(LocalDate.now()) || date.isAfter(LocalDate.now().plusDays(advanceDays))) {
            throw new BusinessException(400, "只能预约今天起 " + advanceDays + " 天内的座位");
        }
        ...
        if (intValue(profile.get("credit_score")) <= threshold) {
            throw new BusinessException(403, "信用积分不足（当前需高于 " + threshold + " 分），暂不可预约");
        }
        ...
        // 校验每日预约次数限制
        int dailyLimit = getConfigInt("reservation_limit_daily", 3);
        Integer todayCount = jdbc.queryForObject("""
                select count(*) from reservation
                where user_id=? and reserve_date=? and status != '已取消'
                """, Integer.class, user.id(), Date.valueOf(date));
        if (todayCount != null && todayCount >= dailyLimit) {
            throw new BusinessException(400, "每日最多只能预约 " + dailyLimit + " 次");
        }
        // 校验单次预约时长限制
        int maxDurationHours = getConfigInt("reservation_limit_duration", 4);
        if (Duration.between(start, end).toMinutes() > maxDurationHours * 60) {
            throw new BusinessException(400, "单次预约时长不能超过 " + maxDurationHours + " 小时");
        }
        LocalDateTime now = LocalDateTime.now();
    ...
    public void cancelReservation(CurrentUser user, Long id) {
        ...
        int cancelPenalty = getConfigInt("credit_cancel_penalty", -50);
        changeCredit(user.id(), cancelPenalty, "USER_CANCEL", "学生主动取消预约", id);
        notifyUser(user.id(), "预约已取消", "你的预约已取消，已扣除 " + Math.abs(cancelPenalty) + " 信用分。", "预约", id);
    }
    ...
    public Map<String, Object> scanCheckin(CurrentUser admin, Map<String, Object> req) {
        ...
        changeCredit(uid, getConfigInt("credit_checkin_reward", 5), CREDIT_ON_TIME, "准时签到奖励", rid);
    ...
    public List<Map<String, Object>> statisticsUsage(CurrentUser admin, String period, Long roomId, String rangeMode) {
        return statisticsUsage(admin, period, roomId, rangeMode, null, null);
    }

    public List<Map<String, Object>> statisticsUsage(CurrentUser admin, String period, Long roomId, String rangeMode,
                                                     String startDate, String endDate) {
        ResolvedStatsRange range = resolveStatsRange(rangeMode, startDate, endDate);
        String extra = sqlStudyRoomScopeForReservationJoin(admin, roomId);
        String dateJoin = reservationDateJoinCondition(period, rangeMode, range);
    ...
    public List<Map<String, Object>> statisticsPeak(CurrentUser admin, String period, Long roomId, String rangeMode) {
        return statisticsPeak(admin, period, roomId, rangeMode, null, null);
    }

    public List<Map<String, Object>> statisticsPeak(CurrentUser admin, String period, Long roomId, String rangeMode,
                                                    String startDate, String endDate) {
        ResolvedStatsRange range = resolveStatsRange(rangeMode, startDate, endDate);
        String dateWhere = reservationDateWhereCondition(period, "r", rangeMode, range);
    ...
    public List<Map<String, Object>> statisticsTrend(CurrentUser admin, String period, Long roomId, String rangeMode) {
        return statisticsTrend(admin, period, roomId, rangeMode, null, null);
    }
    ...
    public List<Map<String, Object>> statisticsTrend(CurrentUser admin, String period, Long roomId, String rangeMode,
        ...
        if ("day".equals(p) && !past) {
            String whereClause = sqlWhereWithRoomScope(admin, "r", reservationDateWhereCondition(p, "r", rangeMode, range), roomId);
        ...
        if (past && "year".equals(p)) {
            String whereClause = sqlWhereWithRoomScope(admin, "r", reservationDateWhereCondition(p, "r", rangeMode, range), roomId);
            String yearSql = sqlJoin("""
                    select year(r.reserve_date) AS yearNum, count(*) AS cnt
                    from reservation r join study_room sr on sr.id=r.room_id""", whereClause,
                    "group by year(r.reserve_date)", "order by year(r.reserve_date)");
            return jdbc.queryForList(yearSql);
        }
        if (past && "month".equals(p)) {
            String whereClause = sqlWhereWithRoomScope(admin, "r", reservationDateWhereCondition(p, "r", rangeMode, range), roomId);
            String monthSql = sqlJoin("""
                    select date_format(r.reserve_date,'%Y-%m') AS timeLabel, count(*) AS cnt
                    from reservation r join study_room sr on sr.id=r.room_id""", whereClause,
                    "group by date_format(r.reserve_date,'%Y-%m')", "order by timeLabel");
            return jdbc.queryForList(monthSql);
        }
        if (past && "week".equals(p)) {
            String whereClause = sqlWhereWithRoomScope(admin, "r", reservationDateWhereCondition(p, "r", rangeMode, range), roomId);
            String weekSql = sqlJoin("""
                    select date_format(r.reserve_date,'%x-%v') AS timeLabel, count(*) AS cnt
                    from reservation r join study_room sr on sr.id=r.room_id""", whereClause,
                    "group by date_format(r.reserve_date,'%x-%v')", "order by timeLabel");
            return jdbc.queryForList(weekSql);
        }
        if ("year".equals(p)) {
            String whereClause = sqlWhereWithRoomScope(admin, "r", reservationDateWhereCondition(p, "r", rangeMode, range), roomId);
            String yearSql = sqlJoin("""
        ...
        String dateWhere = reservationDateWhereCondition(p, "r", rangeMode, range);
        String whereClause = sqlWhereWithRoomScope(admin, "r", dateWhere, roomId);
        String labelFmt = past ? "%Y-%m-%d" : "%m-%d";
        String trendSql = sqlJoin("""
                select date_format(r.reserve_date,'%s') AS timeLabel, count(*) AS cnt
                from reservation r join study_room sr on sr.id=r.room_id""".formatted(labelFmt), whereClause,
    ...
    public Map<String, Object> statisticsReport(CurrentUser admin, String period, Long roomId, String rangeMode,
        ...
        List<Map<String, Object>> usage = statisticsUsage(admin, p, roomId, mode, startDate, endDate);
        List<Map<String, Object>> peak = statisticsPeak(admin, p, roomId, mode, startDate, endDate);
        List<Map<String, Object>> trend = statisticsTrend(admin, p, roomId, mode, startDate, endDate);
        String dateWhere = reservationDateWhereCondition(p, "r", mode, range);
        String whereClause = sqlWhereWithRoomScope(admin, "r", dateWhere, roomId);
        ...
        summary.put("rangeWindowLabel", rangeWindowLabel(p, mode, range));
        if (range != null) {
    ...
    /** 往期窗口说明；若传入自定义区间则展示实际查询范围 */
    private String rangeWindowLabel(String period, String rangeMode, ResolvedStatsRange range) {
        if (range != null) {
            return "统计范围：" + range.start() + " 至 " + range.end();
        }
        ...
        return switch (period == null ? "day" : period.toLowerCase()) {
            case "year" -> "往期窗口：全部历史年份（可自定义区间）";
            case "month" -> "往期窗口：全部历史月份（可自定义区间）";
            case "week" -> "往期窗口：全部历史周次（可自定义区间）";
            default -> "往期窗口：全部历史日期（可自定义区间）";
        };
    }

    private record ResolvedStatsRange(LocalDate start, LocalDate end) {}

    private ResolvedStatsRange resolveStatsRange(String rangeMode, String startDateStr, String endDateStr) {
        if (!isPastRange(rangeMode)) {
            return null;
        }
        LocalDate end = parseStatsDate(endDateStr);
        LocalDate start = parseStatsDate(startDateStr);
        if (end == null) {
            end = latestReservationDate();
        }
        if (start == null) {
            start = earliestReservationDate();
        }
        if (start.isAfter(end)) {
            throw new BusinessException(400, "统计开始日期不能晚于结束日期");
        }
        return new ResolvedStatsRange(start, end);
    }
    ...
    private LocalDate earliestReservationDate() {
        try {
            Date min = jdbc.queryForObject("select min(reserve_date) from reservation", Date.class);
            if (min != null) {
                return min.toLocalDate();
            }
    ...
    private LocalDate latestReservationDate() {
        try {
            Date max = jdbc.queryForObject(
                    "select max(reserve_date) from reservation where reserve_date < current_date()", Date.class);
            if (max != null) {
                return max.toLocalDate();
            }
            max = jdbc.queryForObject("select max(reserve_date) from reservation", Date.class);
            if (max != null) {
                return max.toLocalDate();
            }
        } catch (Exception ignored) {
    ...
    private String reservationDateJoinCondition(String period, String rangeMode, ResolvedStatsRange range) {
        return reservationDateWhereCondition(period, "r", rangeMode, range);
    }

    private String reservationDateWhereCondition(String period, String alias, String rangeMode, ResolvedStatsRange range) {
        return statsDateWhereCondition(period, alias + ".reserve_date", rangeMode, range);
    }

    private String statsDateWhereCondition(String period, String col, String rangeMode, ResolvedStatsRange range) {
        if (range != null) {
            return col + " between '" + range.start() + "' and '" + range.end() + "'";
        }
        String p = period == null ? "day" : period.toLowerCase();
        boolean past = isPastRange(rangeMode);
        return switch (p) {
    ...
    public List<Map<String, Object>> statisticsCreditChanges(CurrentUser admin, String period, Long roomId,
                                                             String rangeMode, String startDate, String endDate) {
        ResolvedStatsRange range = resolveStatsRange(rangeMode, startDate, endDate);
        String whereClause = sqlJoin("where", statsDateWhereCondition(period, "date(cl.created_at)", rangeMode, range));
        if (roomId != null) {
            assertRoomStatsAccess(admin, roomId);
            whereClause = sqlJoin(whereClause, "and", "r.room_id=" + roomId);
        } else if (!admin.isSuperAdmin()) {
            whereClause = sqlJoin(whereClause, "and", "sr.manager_id=" + admin.id());
        }
        String changeSql = sqlJoin("""
                select coalesce(cl.change_type, '其他') changeType,
                       count(*) recordCount,
                       coalesce(sum(case when cl.change_value>0 then cl.change_value else 0 end),0) increaseScore,
                       coalesce(sum(case when cl.change_value<0 then abs(cl.change_value) else 0 end),0) decreaseScore
                from credit_log cl
                left join reservation r on r.id=cl.reservation_id
                left join study_room sr on sr.id=r.room_id""",
                whereClause,
                "group by coalesce(cl.change_type, '其他')",
                "order by recordCount desc, changeType");
        return jdbc.queryForList(changeSql);
    }

    public List<Map<String, Object>> statisticsViolationSummary(CurrentUser admin, String period, Long roomId,
                                                                String rangeMode, String startDate, String endDate) {
        ResolvedStatsRange range = resolveStatsRange(rangeMode, startDate, endDate);
        String dateWhere = reservationDateWhereCondition(period, "r", rangeMode, range);
        String whereClause = sqlWhereWithRoomScope(admin, "r", dateWhere, roomId);
        String violationSql = sqlJoin("""
                select sr.name roomName,
                       count(*) violationCount,
                       count(distinct r.user_id) affectedStudents
                from reservation r
                join study_room sr on sr.id=r.room_id""",
                whereClause,
                "and", violatedReservationPredicate("r"),
                "group by sr.id,sr.name",
                "order by violationCount desc, sr.id");
        return jdbc.queryForList(violationSql);
    }

    public List<Map<String, Object>> statisticsStudentActivity(CurrentUser admin, String period, Long roomId,
                                                               String rangeMode, String startDate, String endDate) {
        ResolvedStatsRange range = resolveStatsRange(rangeMode, startDate, endDate);
        String dateWhere = reservationDateWhereCondition(period, "r", rangeMode, range);
        String whereClause = sqlWhereWithRoomScope(admin, "r", dateWhere, roomId);
        String minutesExpr = durationMinutesExpression("r");
        String activitySql = sqlJoin("""
                select sp.student_no studentNo,
                       sp.name studentName,
                       sp.college college,
                       sp.major major,
                       sp.grade grade,
                       sp.credit_score creditScore,
                       count(distinct r.id) reservationCount,
                       count(distinct case when %s then r.id end) checkinCount,
                       count(distinct case when %s then r.id end) violationCount,
                       coalesce(sum(case when %s then %s else 0 end), 0) studyMinutes
                from reservation r
                join student_profile sp on sp.user_id=r.user_id
                join study_room sr on sr.id=r.room_id""".formatted(
                        usedReservationPredicate("r"),
                        violatedReservationPredicate("r"),
                        usedReservationPredicate("r"),
                        minutesExpr),
                whereClause,
                "group by sp.user_id,sp.student_no,sp.name,sp.college,sp.major,sp.grade,sp.credit_score",
                "order by reservationCount desc, studyMinutes desc, sp.student_no");
        return jdbc.queryForList(activitySql);
    }
    ...
    public void scheduledProcessNoShow() {
        ...
        for (Map<String, Object> row : rows) {
            ...
            releaseReservationSlots(id);
            int penalty = getConfigInt("credit_violation_penalty", -50);
            changeCredit(userId, penalty, CREDIT_VIOLATION, "预约超时未签到", id);
            notifyUser(userId, "预约违约", "你有一条预约因超时未签到被判定违约，已扣除 " + Math.abs(penalty) + " 信用分。", "VIOLATION", id);
        }
    }
    ...
    public String exportCsv(CurrentUser admin, String period, Long roomId, String rangeMode) {
        return exportCsv(admin, period, roomId, rangeMode, null, null);
    }

    public String exportCsv(CurrentUser admin, String period, Long roomId, String rangeMode,
                            String startDate, String endDate) {
        return exportCsv(admin, period, roomId, rangeMode, startDate, endDate, "usage");
    }
    ...
    public String exportCsv(CurrentUser admin, String period, Long roomId, String rangeMode,
        ...
        StringBuilder csv = exportHeader(admin, type, p, roomId, rangeMode, range);
        switch (type) {
            case "reservation" -> appendReservationTrendCsv(csv, admin, p, roomId, rangeMode, startDate, endDate);
            case "peak" -> appendPeakCsv(csv, admin, p, roomId, rangeMode, startDate, endDate);
            case "activity" -> appendStudentActivityCsv(csv, admin, p, roomId, rangeMode, startDate, endDate);
            case "credit" -> appendCreditCsv(csv, admin, p, roomId, rangeMode, startDate, endDate);
            case "studyDuration" -> appendStudyDurationCsv(csv, admin, p, roomId, rangeMode, startDate, endDate);
            default -> appendUsageCsv(csv, admin, p, roomId, rangeMode, startDate, endDate);
        }
        return csv.toString();
    }

    public String statisticsExportFilename(String reportType) {
        return normalizeReportType(reportType) + "-report.csv";
    }

    private StringBuilder exportHeader(CurrentUser admin, String reportType, String period, Long roomId, String rangeMode, ResolvedStatsRange range) {
        String label = periodLabel(period, rangeMode);
        StringBuilder csv = new StringBuilder("报表类型,").append(reportTypeLabel(reportType)).append("\n");
        csv.append("时间维度,").append(label).append("\n");
        csv.append(rangeWindowLabel(period, rangeMode, range)).append("\n");
        if (roomId != null) {
            assertRoomStatsAccess(admin, roomId);
            Map<String, Object> roomInfo = room(roomId);
    ...
    private void appendUsageCsv(StringBuilder csv, CurrentUser admin, String period, Long roomId, String rangeMode,
                                String startDate, String endDate) {
        csv.append("自习室,总座位,预约数,实际使用数,使用率\n");
        for (Map<String, Object> row : statisticsUsage(admin, period, roomId, rangeMode, startDate, endDate)) {
    ...
    private void appendReservationTrendCsv(StringBuilder csv, CurrentUser admin, String period, Long roomId,
                                           String rangeMode, String startDate, String endDate) {
        csv.append("统计维度,预约数\n");
        for (Map<String, Object> row : statisticsTrend(admin, period, roomId, rangeMode, startDate, endDate)) {
            csv.append(trendLabel(row)).append(',')
                    .append(row.get("cnt")).append("\n");
        }
    }

    private void appendPeakCsv(StringBuilder csv, CurrentUser admin, String period, Long roomId,
                               String rangeMode, String startDate, String endDate) {
        csv.append("开始时段,预约数\n");
        for (Map<String, Object> row : statisticsPeak(admin, period, roomId, rangeMode, startDate, endDate)) {
            csv.append(hourLabel(row.get("peakHour"))).append(',')
                    .append(row.get("cnt")).append("\n");
        }
    }

    private void appendStudentActivityCsv(StringBuilder csv, CurrentUser admin, String period, Long roomId,
                                          String rangeMode, String startDate, String endDate) {
        csv.append("学号,姓名,学院,专业,年级,预约次数,签到次数,违约次数,累计自习分钟,累计自习小时,当前信用分\n");
        for (Map<String, Object> row : statisticsStudentActivity(admin, period, roomId, rangeMode, startDate, endDate)) {
            double hours = intValue(row.get("studyMinutes")) / 60.0;
            csv.append(row.get("studentNo")).append(',')
                    .append(row.get("studentName")).append(',')
                    .append(row.get("college")).append(',')
                    .append(row.get("major")).append(',')
                    .append(row.get("grade")).append(',')
                    .append(row.get("reservationCount")).append(',')
                    .append(row.get("checkinCount")).append(',')
                    .append(row.get("violationCount")).append(',')
                    .append(row.get("studyMinutes")).append(',')
                    .append(String.format(Locale.ROOT, "%.1f", hours)).append(',')
                    .append(row.get("creditScore")).append("\n");
        }
    }

    private void appendCreditCsv(StringBuilder csv, CurrentUser admin, String period, Long roomId,
                                 String rangeMode, String startDate, String endDate) {
        csv.append("当前信用分布\n");
        csv.append("信用等级,人数\n");
        for (Map<String, Object> row : statisticsCredit()) {
            csv.append(row.get("label")).append(',')
                    .append(row.get("value")).append("\n");
        }
        csv.append("\n周期内信用变动\n");
        csv.append("变动类型,记录数,加分总计,扣分总计\n");
        for (Map<String, Object> row : statisticsCreditChanges(admin, period, roomId, rangeMode, startDate, endDate)) {
            csv.append(row.get("changeType")).append(',')
                    .append(row.get("recordCount")).append(',')
                    .append(row.get("increaseScore")).append(',')
                    .append(row.get("decreaseScore")).append("\n");
        }
        csv.append("\n周期内违约统计\n");
        csv.append("自习室,违约次数,受影响学生数\n");
        for (Map<String, Object> row : statisticsViolationSummary(admin, period, roomId, rangeMode, startDate, endDate)) {
            csv.append(row.get("roomName")).append(',')
                    .append(row.get("violationCount")).append(',')
                    .append(row.get("affectedStudents")).append("\n");
        }
    }

    private void appendStudyDurationCsv(StringBuilder csv, CurrentUser admin, String period, Long roomId,
                                        String rangeMode, String startDate, String endDate) {
        csv.append("学号,姓名,累计自习分钟,累计自习小时,签到次数\n");
        for (Map<String, Object> row : statisticsStudentActivity(admin, period, roomId, rangeMode, startDate, endDate)) {
            double hours = intValue(row.get("studyMinutes")) / 60.0;
            csv.append(row.get("studentNo")).append(',')
                    .append(row.get("studentName")).append(',')
    ...
    private String normalizeReportType(String reportType) {
        ...
        return switch (type) {
            case "reservation", "peak", "activity", "credit", "studyDuration" -> type;
            default -> "usage";
        };
    }
    ...
    private String reportTypeLabel(String reportType) {
        return switch (normalizeReportType(reportType)) {
            case "reservation" -> "预约统计报表";
            case "peak" -> "高峰时段报表";
            case "activity" -> "用户活跃度报表";
            case "credit" -> "信用积分报表";
            case "studyDuration" -> "自习时长报表";
            default -> "座位使用率报表";
        };
    }

    private String trendLabel(Map<String, Object> row) {
        if (row.get("peakHour") != null) {
            return hourLabel(row.get("peakHour"));
        }
        if (row.get("monthNum") != null) {
            return row.get("monthNum") + "月";
        }
        if (row.get("yearNum") != null) {
            return String.valueOf(row.get("yearNum"));
        }
        return String.valueOf(row.getOrDefault("timeLabel", ""));
    }

    private String hourLabel(Object hour) {
        return String.format(Locale.ROOT, "%02d:00", intValue(hour));
    }
